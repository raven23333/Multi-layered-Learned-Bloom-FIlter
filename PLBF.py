...
def get_thresholds(key_scores, non_key_scores, num_partitions):
    ...
    # Larger chunks might lead to an error be careful
    ...
    for i in range(0, num_chunks):
        if G_cummulative[i] == 0:
            DP_matrix[0][i] = 0
        elif H_cummulative[i] == 0: # TODO
    ...
    try:
        ...
        print("warning: bad dataset")
        threshold_params = list(chunk_dict.values())[1:]
...
def get_parameter_vals(key_scores, non_key_scores, target_fpr, num_partitions):
    ...
    while len(fpr_params) > 2 and fpr_params[0] <= 0 and fpr_params[1] <= 0:
        threshold_params = threshold_params[1:]
        fpr_params = fpr_params[1:]
    while len(fpr_params) > 2 and fpr_params[-1] >= 1 and fpr_params[-2] >= 1:
        threshold_params = threshold_params[:-1]
        fpr_params = fpr_params[:-1]
    if len(fpr_params) == 0 and len(threshold_params) == 0:
        return [0.99, 1], [0, 1]
    if fpr_params is [] or threshold_params is []:
        print("sdf")
    if threshold_params[-1] < 1 and fpr_params[-1] >= 1:
        fpr_params[-1] = 1
        threshold_params[-1] = 1
    if threshold_params[0] > 0:
        threshold_params = [0] + threshold_params
    # print("Threshold params:", threshold_params)
    # print("fpr_params:", fpr_params)
